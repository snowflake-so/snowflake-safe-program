...
mod tests {
  ...
  fn test_invalid_patterns() {
    assert!(matches!(parse_cron("* * * * * *").unwrap_err(), CrontabError::ErrCronFormat { .. }));
    assert!(matches!(parse_cron("* * 15").unwrap_err(), CrontabError::ErrCronFormat { .. }));
    assert!(matches!(parse_cron("9 * * * * *").unwrap_err(), CrontabError::ErrCronFormat { .. }));
  }
  ...
  fn test_outside_ranges() {
    assert!(matches!(parse_cron("65 * * * *").unwrap_err(), CrontabError::FieldOutsideRange { .. }));
    assert!(matches!(parse_cron("* 25 * * *").unwrap_err(), CrontabError::FieldOutsideRange { .. }));
    assert!(matches!(parse_cron("* * 32 * *").unwrap_err(), CrontabError::FieldOutsideRange { .. }));
    assert!(matches!(parse_cron("* * * 13 *").unwrap_err(), CrontabError::FieldOutsideRange { .. }));
    assert!(matches!(parse_cron("* * * * 8").unwrap_err(), CrontabError::FieldOutsideRange { .. }));
  }

  #[test]
  fn test_invalid_steps() {
    let result = std::panic::catch_unwind(|| parse_cron("*/2 *////343433 * * * "));
    assert!(matches!(result.unwrap(), Err(CrontabError::ErrParseInt { .. })));
  }
}
